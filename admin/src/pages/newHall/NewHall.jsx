...
const NewHall = () => {
  ...
  const {data,loading,error}=useFetch(`${window.location.origin}/api/slots`);
  ...
  const handleClick= async (e)=>{
    ...
    try{
      const list=await Promise.all(
        ...
        Object.values(files).map(async (file)=>{
          ...
          const uploadRes=await axios.post(
            "https://api.cloudinary.com/v1_1/dqvnsta8e/image/upload",
            data
          )
      ...
      await axios.post(`${window.location.origin}/api/halls`,newHall)
      }catch(err){}
  }
  return (
    <div className="new">
